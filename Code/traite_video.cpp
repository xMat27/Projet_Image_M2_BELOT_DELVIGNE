...
int main(int argc, char* argv[])
{
  ...
  VideoWriter videoTraite(cNomVideoTraite, VideoWriter::fourcc('X', '2', '6', '4'), fps, Size(frame_width, frame_height), true);
  int nbFrame = 0;
  ...
  while (true) {
      ...
      for (int x = 0; x < frame.rows; x++) {
        for (int y = 0; y < frame.cols; y++) {
            ...
            ImgOut[x*frame_width*3 + y*3+1] = ImgIn[x*frame_width*3 + y*3+0];
            ImgOut[x*frame_width*3 + y*3+2] = ImgIn[x*frame_width*3 + y*3+0];
        }
      }
