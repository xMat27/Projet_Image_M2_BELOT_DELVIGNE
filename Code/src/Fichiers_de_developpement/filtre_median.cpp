  ...
  while (true) {
      ...
      for (int x = 1; x < frame.rows - 1; x++) {
        for (int y = 1; y < frame.cols - 1; y++) {
            medR = {ImgIn[x*frame_width*3 + y*3+0], ImgIn[(x+1)*frame_width*3 + y*3+0], ImgIn[(x-1)*frame_width*3 + y*3+0], ImgIn[(x+1)*frame_width*3 + (y+1)*3+0], ImgIn[(x-1)*frame_width*3 + (y-1)*3+0], ImgIn[(x)*frame_width*3 + (y+1)*3+0], ImgIn[(x+1)*frame_width*3 + (y-1)*3+0], ImgIn[(x-1)*frame_width*3 + (y+1)*3+0], ImgIn[(x)*frame_width*3 + (y-1)*3+0]};
            medG = {ImgIn[x*frame_width*3 + y*3+1], ImgIn[(x+1)*frame_width*3 + y*3+1], ImgIn[(x-1)*frame_width*3 + y*3+1], ImgIn[(x+1)*frame_width*3 + (y+1)*3+1], ImgIn[(x-1)*frame_width*3 + (y-1)*3+1], ImgIn[(x)*frame_width*3 + (y+1)*3+1], ImgIn[(x+1)*frame_width*3 + (y-1)*3+1], ImgIn[(x-1)*frame_width*3 + (y+1)*3+1], ImgIn[(x)*frame_width*3 + (y-1)*3+1]};
            medB = {ImgIn[x*frame_width*3 + y*3+2], ImgIn[(x+1)*frame_width*3 + y*3+2], ImgIn[(x-1)*frame_width*3 + y*3+2], ImgIn[(x+1)*frame_width*3 + (y+1)*3+2], ImgIn[(x-1)*frame_width*3 + (y-1)*3+2], ImgIn[(x)*frame_width*3 + (y+1)*3+2], ImgIn[(x+1)*frame_width*3 + (y-1)*3+2], ImgIn[(x-1)*frame_width*3 + (y+1)*3+2], ImgIn[(x)*frame_width*3 + (y-1)*3+2]};

            std::sort(medR.begin(), medR.end());
            std::sort(medG.begin(), medG.end());
            std::sort(medB.begin(), medB.end());
            ImgOut[x*frame_width*3 + y*3+0] = medR[4];
            ImgOut[x*frame_width*3 + y*3+1] = medG[4];
            ImgOut[x*frame_width*3 + y*3+2] = medB[4];
        }
      }
      ...
      imwrite("frame_median.jpg", frameTraite);
